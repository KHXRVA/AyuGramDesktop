...
AyuGram Desktop со слитым апстримом Telegram Desktop 7.2.9 (исправлено «сообщение не поддерживается» для сообщений с кнопками). Добавлено:

**Удалённые сообщения**
- остаются в чате после перезапуска, вместе с медиа
- отдельные переключатели для сохранения удалённых в каналах и в комментариях
- исключения по чатам: «Сохранять удалёнки» / «Не сохранять удалёнки» в меню чата, список исключений в настройках
- массовая очистка: для выделенных в списке чатов или сразу во всех каналах
- переход к дате (календарь) в просмотре удалённых

**Реакции**
- закреплённые реакции для чатов и каналов, поддержка кастомных эмодзи, панель эмодзи в редакторе
- в быстрой полоске всегда 7 реакций плюс стрелка, кастомные эмодзи анимируются, порядок сохраняется как задан
- «Убрать все мои реакции» в меню чата

**Message Shot**
- юзернеймы вместо имён (без @), режимы аватарок, блюр аватарок и имён
- выравнивание сообщений по общим границам
- стили: Classic, Cards, Code window; градиентные фоны

**Кружки**
- размер кружков в чате: 100–300%
- редактор перед отправкой: записанный кружок можно обрезать так же, как голосовое, ролик перекодируется в выбранный диапазон

**Копирование**
- Ctrl+C: одно сообщение без меты, несколько — как чат-лог
- Ctrl+Shift+C: всегда с метой (дата, время, автор, на что ответ)
- «Копировать как чат-лог» в расширенном меню сообщения (Ctrl/Shift + ПКМ)

**Прочее**
- скрытие ответа при пересылке
- скрытие Кошелька в меню

Community-сборка: конфигурировать с `-DAYU_COMMUNITY_BUILD=ON` (тег, автор и ссылка — в `Telegram/cmake/telegram_options.cmake`). В ней автор сборки показан в главном меню, окне «О программе», строке версии, свойствах exe и шапке настроек AyuGram.
