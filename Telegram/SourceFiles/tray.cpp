...
void Tray::rebuildMenu() {
...

	{
		auto minimizeText = _textUpdates.events(
			...
			return _activeForTrayIconAction
				...
				: tr::lng_open_from_tray(tr::now).replace("Telegram", AyuSettings::getInstance().effectiveAppName());
		});
	...
	{
		return tr::lng_quit_from_tray(tr::now).replace("Telegram", AyuSettings::getInstance().effectiveAppName());
	});
